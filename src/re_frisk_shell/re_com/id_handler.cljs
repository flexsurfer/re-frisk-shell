(ns re-frisk-shell.re-com.id-handler
...
(defn re-frame-event [kind->id->handler]
  (into {} (map (fn [a]
                  (hash-map (first a)
                            (let [intc (map #(merge {(:id %1) (cond (and (= %2 0) (= (:id %1) :coeffects)) (:coeffects intrcp)
                              ...
                              (hash-map (str (count intc) " interceptors")
                                        intc))))
                (filter #(not= (key %) :re-frisk/update-db) (:event kind->id->handler)))))

(defn re-frame-sub [kind->id->handler]
  (into {} (map #(let [k (first %)]
                   (hash-map k (subscribe [k])))
                (filter #(not= (first %) ::db) (:sub kind->id->handler)))))

(defn re-frame-fx [kind->id->handler]
  (map #(if (% fx) {% (% fx)} %) (keys (:fx kind->id->handler))))

(defn re-frame-cofx [kind->id->handler]
  (map #(if (% cofx) {% (% cofx)} %) (keys (:cofx kind->id->handler))))

(defn re-frame-handlers [kind->id->handler]
  (let [event (reaction (re-frame-event kind->id->handler))
        sub (reaction (re-frame-sub kind->id->handler))]
    {:event {(count @event) event}
     :sub {(count @sub) sub}
     :fx (reaction (re-frame-fx kind->id->handler))
     :cofx (reaction (re-frame-cofx kind->id->handler))}))
